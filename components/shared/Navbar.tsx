...
import {
  useAppSelector,
  useAppDispatch,
} from "@/app/[locale]/globalRedux/hooks";
import Blockies from "react-blockies";

import { loadAccount } from "@/app/[locale]/globalRedux/interactions";
import { setAccount } from "@/app/[locale]/globalRedux/features/connectionSlice";
...
function Navbar() {
  ...
  return (
    <nav
      className={`py-3 ${
        (pathname.startsWith("/aml") ||
          pathname.startsWith("/terms") ||
          pathname.startsWith("/privacy")) &&
        "bg-[#f0f2f4]"
      } ${
        (pathname.startsWith("/exchange") ||
          pathname.startsWith("/en/exchange")) &&
        "bg-black text-white"
      }`}
    >
      {isMenu === false ? (
        <div className="container mx-auto max-w-7xl flex items-center justify-between px-10">
          <Link href="/" className="flex items-center">
            ...
            <p className="max-xs:hidden font-semibold text-[25px]">
              Abu<span className="text-primary">Global</span>
            </p>
          </Link>
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden md:flex gap-4 font-semibold leading-5 items-center">
              <Link
                href={"/contact"}
                className="border-b-2 border-transparent transition-border-color duration-300 ease hover:border-primary"
              >
                Contact
              </Link>
              {(pathname.startsWith("/exchange") ||
              ...
              address ? (
                  ...
                  <Link
                    href={
                      (config as any)[chainId]
                        ? `${
                            (config as any)[chainId].explorerURL
                          }/address/${address}`
                        : "#"
                    }
                    target="_blank"
                ...
                pathname.startsWith("/en/exchange") ? (
                <button
                  className="flex flex-row gap-2 justify-center items-center bg-primary text-white px-5 py-3 rounded-xl shadow-md drop-shadow-md transition-opacity duration-200 ease hover:brightness-90"
                ...
                <Link href={"/exchange"} className="w-full">
                  <Button
                    ...
                    styles="bg-primary text-white px-5 py-3 shadow-md"
                  />
                </Link>
              )}
            </div>
        ...
        <div className="md:hidden fixed top-0 left-0 w-full h-full mx-auto flex flex-col justify-start gap-4 text-white z-50 bg-gradient-to-b from-black via-black to-primary">
          ...
          <div className="flex flex-col gap-4 px-8">
            <div className="flex flex-col gap-8 font-semibold leading-5 justify-start">
              <Link
                href={"/contact"}
                className="text-[30px] transition-opacity duration-400 ease-in hover:opacity-20"
              >
                Contact
              </Link>
              {(pathname.startsWith("/exchange") ||
              ...
              address ? (
                ...
                <Link href={"/exchange"} className="w-full">
                  <Button
                    text="Buy & sell crypto"
                    styles="bg-primary text-white px-5 py-3"
                  />
                </Link>
              )}
            </div>
