...
import {
  loadTokens,
  loadAccount,
} from "@/app/[locale]/globalRedux/interactions";
...
import {
  ...
  setChainId,
  setAccount,
} from "@/app/[locale]/globalRedux/features/connectionSlice";
import { setTokens } from "@/app/[locale]/globalRedux/features/tokensSlice";
...
const config: Record<string, ChainConfig> = configJson as Record<
  string,
  ChainConfig
>;
  ...
  const loadBlockchainData = async () => {
    try {
      if (typeof window !== "undefined" && window.ethereum) {
        ...
        const tokens = await loadTokens(loadProvider, [
          USDT.address,
          USDC.address,
        ]);
        ...
        console.error(
          "Provider not detected. Please install MetaMask or a similar wallet."
        );
      }
    } catch (error) {
  ...
  return (
    <section className="relative bg-cover bg-[url('/assets/gradient.png')]">
      <div className="container mx-auto max-w-7xl flex flex-col items-center justify-between px-10 py-20">
        ...
        <h1 className="text-white font-bold text-[36px] md:text-[48px] mt-10">
          Buy & Sell Crypto
        </h1>
      </div>
    </section>
